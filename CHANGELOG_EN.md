...
- **Nushell (`nu`) is no longer treated as bash**: `detectShellKind` used `base.includes('nu')` and folded Nushell into the bash family, so launch paths were assembled with POSIX rules — but Nushell requires the **`^` sigil for external commands** (without it the name is parsed as an internal command and fails), and its single-quoted strings take no escapes and cannot contain a quote. `nu` is now its own shell kind: the command position gets `^`, and values are quoted as raw strings (`r#'…'#`), lengthening the delimiter when the value contains `'#`. Note: `nu` is not installed on this machine — the implementation follows Nushell's official documentation and has **not been verified against a real Nushell**.
