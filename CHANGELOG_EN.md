...
- **Nushell (`nu`) is no longer treated as bash**: `detectShellKind` used `base.includes('nu')` and folded Nushell into the bash family, so launch paths were assembled with POSIX rules — but Nushell's single-quoted strings take no escapes and cannot contain a quote at all (the POSIX `'\''` splice is invalid there), and on Windows it must not take the extensionless POSIX shim (Nushell cannot execute it). `nu` is now its own shell kind: it resolves to `.cmd`/`.exe`, the command position gets `^`, and values are quoted as raw strings (`r#'…'#`), lengthening the delimiter when the value contains `'#`. Verified against a real Nushell (0.115.1, plus a 0.94–0.110 version matrix): the assembled command executes correctly and arguments arrive character-for-character.
