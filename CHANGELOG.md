...
- **启动命令与 `@` 引用里的路径转义**：工作区路径、启动路径中的 shell 元字符（`;` `&` `|` `$`、内嵌引号等）此前不被转义——判据是「是否含空格」，于是 `/tmp/repo;id` 会作为两条命令抵达 shell。现在按目标 shell 分别引用并转义；普通路径的输出保持不变。
- **超大选区不再撑爆 IDE 通道**：推送给 dsh-tui 的选区文本此前没有上限，整选一个数 MB 的文件会产生超大帧并断开 WebSocket——而 dsh-tui 断链后静默降级且不重连，该会话的选区通道就此永久失效。现按 20 万字符封顶（高于 TUI 自身 5 万的上限，好让它仍能渲染截断标记）。
- **禁用 `autoInsertMention` 后清除 dsh-tui 侧的选区**：此前禁用只停止推送，TUI 已收到的选区快照会残留并附加到下一次提交。现在禁用时补推一次空选区通知。
- **IDE server 的停用竞态**：`start()` 是异步的，扩展若在它落定前停用，`stop()` 会因为内部句柄尚未建立而直接返回，留下无人接管的服务端。停用现在等启动落定。
