...
- **Nushell（`nu`）不再被当作 bash**：`detectShellKind` 原先用 `base.includes('nu')` 把 Nushell 吞进 bash 家族，启动路径于是按 POSIX 规则拼——而 Nushell 的**外部命令必须带 `^` 前缀**（否则被当内部命令解析并报错），且它的单引号字符串既不支持转义、也不能内含单引号。现在 `nu` 是独立的 shell 类型：命令位加 `^`，字符串用 raw string（`r#'…'#`），含 `'#` 序列时按需加长分隔符。注意：本机未安装 `nu`，此实现依据 Nushell 官方文档，**未经真实 Nushell 实测**。
