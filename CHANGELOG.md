...
- **Nushell（`nu`）不再被当作 bash**：`detectShellKind` 原先用 `base.includes('nu')` 把 Nushell 吞进 bash 家族，启动路径于是按 POSIX 规则拼装——而 Nushell 的单引号字符串既不支持转义、也不能内含单引号（POSIX 的 `'\''` 拼接在那里语法无效），Windows 下也不该走无扩展名的 POSIX shim（Nushell 执行不了它）。现在 `nu` 是独立的 shell 类型：解析到 `.cmd`/`.exe`、命令位加 `^`、字符串用 raw string（`r#'…'#`），含 `'#` 序列时按需加长分隔符。已用真实 Nushell（0.115.1，另测 0.94–0.110 版本矩阵）实测：拼出的命令能正确执行，参数逐字符送达。
