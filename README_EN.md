...
- **Automatic selection context (on by default)**: editor selections are pushed in real time to the running dsh-tui over the **IDE selection channel** (300 ms debounce, coordinates only, never touching the input box); asking a question automatically attaches the selected content and shows a "⧉ Selected N lines from …" indicator line (mirroring the official selection_changed behavior); when the channel is unavailable it falls back to typing `@relative/path#L…`. Requires a dsh-TUI build that includes the IDE selection channel (after upstream merges #562).
